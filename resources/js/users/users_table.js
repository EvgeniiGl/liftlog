var TableModule = (function() {
	...
	return {
		...
		events: function() {
			...
			// });
			table.on('click', '.btn-access', function(e) {
				$(window).trigger('open_form_access',e.currentTarget.dataset.id);
			});
		},
		getDataTr: function() {
			if (!selectedTr) {
		...
		// 		type: 'POST',
		// 		url: 'users/access',
		// 		data: { id: btn.data('id'), access: btn.attr('data-access') },
		// 		success: function(data) {
		// 			if(data.status) {
		// 				btn.attr('data-access', data.access?0:1);
		// 				btn.removeClass('btn-success btn-danger');
		// 				btn.addClass(data.access ? 'btn-success' : 'btn-danger');
		// 				btn.text(data.access ?  'включен':'отключен' );
		// 			}
		// 		},
		// 		error: function(data) {
